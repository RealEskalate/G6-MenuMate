...
func (s *stubUserRepo) CreateUser(ctx context.Context, u *domain.User) error { s.users = append(s.users, *u); return nil }
func (s *stubUserRepo) FindUserByID(ctx context.Context, id string) (*domain.User, error) { return nil, domain.ErrNotFound }
func (s *stubUserRepo) GetUserByUsername(ctx context.Context, username string) (*domain.User, error) { return nil, domain.ErrNotFound }
func (s *stubUserRepo) GetUserByEmail(ctx context.Context, email string) (*domain.User, error) { return nil, domain.ErrNotFound }
func (s *stubUserRepo) GetUserByPhone(ctx context.Context, phone string) (*domain.User, error) { return nil, domain.ErrNotFound }
...
func (s *stubUserRepo) FindByUsernameOrEmail(ctx context.Context, key string) (domain.User, error) {
    for _, u := range s.users {
        if u.Username == key || u.Email == key || u.PhoneNumber == key { return u, nil }
    }
...
func TestFindByIdentifier_UserEmailUsernamePhone(t *testing.T) {
    ...
    for _, c := range cases {
        ...
        if err != nil { t.Fatalf("expected success for %s lookup, got error: %v", c.field, err) }
        if got.ID != user.ID { t.Fatalf("expected user ID %s for %s lookup, got %s", user.ID, c.field, got.ID) }
    }
