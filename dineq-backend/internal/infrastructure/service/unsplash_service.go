...
type unsplashSearchResponse struct { Results []unsplashPhoto `json:"results"` }
type unsplashPhoto struct { ID string `json:"id"`; Urls unsplashPhotoUrls `json:"urls"`; Alt string `json:"alt_description"` }
type unsplashPhotoUrls struct { Full string `json:"full"`; Thumb string `json:"thumb"` }
...
func (s *UnsplashSearchService) Search(ctx context.Context, itemName string, num int) ([]PhotoMatch, error) {
    ...
    if err != nil { return nil, fmt.Errorf("unsplash request build: %w", err) }
    req.Header.Set("Authorization", "Client-ID "+s.apiKey)
    resp, err := s.client.Do(req)
    if err != nil { return nil, fmt.Errorf("unsplash request: %w", err) }
    defer resp.Body.Close()
    if resp.StatusCode != http.StatusOK { return nil, fmt.Errorf("unsplash status %d", resp.StatusCode) }
    var out unsplashSearchResponse
    if err := json.NewDecoder(resp.Body).Decode(&out); err != nil { return nil, fmt.Errorf("unsplash decode: %w", err) }
