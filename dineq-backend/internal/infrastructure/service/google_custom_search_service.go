...
func (g *GoogleCustomSearchService) SearchFoodImages(ctx context.Context, itemName, restaurantName string, limit int) ([]PhotoMatch, error) {
    ...
    if err != nil { return nil, fmt.Errorf("execute search: %w", err) }
...
func (g *GoogleCustomSearchService) confidence(itemName, title, snippet string) float64 {
    ...
    score := 0.5
    if strings.Contains(titleLower, itemLower) { score += 0.3 }
    for _, w := range strings.Fields(itemLower) {
        if len(w) > 2 && strings.Contains(titleLower, w) { score += 0.1 }
    }
    if strings.Contains(snippetLower, itemLower) { score += 0.2 }
    ethiopian := []string{"ethiopian", "injera", "berbere", "doro", "kitfo", "tibs", "shiro"}
    for _, k := range ethiopian { if strings.Contains(titleLower, k) || strings.Contains(snippetLower, k) { score += 0.15; break } }
