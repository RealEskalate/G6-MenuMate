...
type menuProcessingResults struct {
	MenuItems []menuItemResult `json:"menuItems"`
}
	...
	for attempt := 1; attempt <= 3; attempt++ {
		...
		for _, mi := range results.MenuItems {
			...
			for i := range t.Categories {
				if t.Categories[i].Name == catName {
					cat = &t.Categories[i]
					break
				}
			}
			if cat == nil {
				t.Categories = append(t.Categories, domain.Category{ID: bson.NewObjectID().Hex(), TabID: t.ID, Name: catName, NameAm: amCat})
				cat = &t.Categories[len(t.Categories)-1]
			}
			...
			if m, ok := mi.NutritionalInfo.(map[string]any); ok {
				if v, ok2 := m["calories"]; ok2 {
					if f, ok3 := toInt(v); ok3 {
						calories = f
					}
				}
				if v, ok2 := m["protein"]; ok2 {
					if f, ok3 := toInt(v); ok3 {
						protein = f
					}
				}
				if v, ok2 := m["carbs"]; ok2 {
					if f, ok3 := toInt(v); ok3 {
						carbs = f
					}
				}
...
func (gs *GeminiService) parseGeminiResponse(response string) (*menuProcessingResults, error) {
	...
	if !strings.HasPrefix(response, "{") || !strings.Contains(response, "menuItems") {
		if start := strings.Index(response, "{"); start != -1 {
			if end := strings.LastIndex(response, "}"); end != -1 && end > start {
				...
				if strings.Contains(candidate, "menuItems") {
					response = candidate
				}
			}
		}
	...
	if err := dec.Decode(&results); err != nil {
		...
		if start != -1 && end != -1 && end > start {
			...
			if err2 := dec2.Decode(&results); err2 == nil {
				return &results, nil
			} else {
				return nil, fmt.Errorf("failed to decode JSON response: %v, response: %s", err2, normalized)
			}
		}
...
func (gs *GeminiService) TranslateAIBit(text, target string) (string, error) {
	...
	if len(resp.Candidates) == 0 || len(resp.Candidates[0].Content.Parts) == 0 {
		return text, fmt.Errorf("no translation response from Gemini")
	}
	var out string
...
func transient(err error) bool {
	...
	if strings.Contains(msg, "503") || strings.Contains(strings.ToLower(msg), "unavailable") {
		return true
	}
