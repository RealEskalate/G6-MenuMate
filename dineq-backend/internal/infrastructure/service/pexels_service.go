...
type pexelsSearchResponse struct { Photos []pexelsPhoto `json:"photos"` }
type pexelsPhoto struct { ID int `json:"id"`; Src pexelsPhotoSrc `json:"src"`; Alt string `json:"alt"` }
type pexelsPhotoSrc struct { Original string `json:"original"`; Small string `json:"small"`; Medium string `json:"medium"`; Large string `json:"large"`; Tiny string `json:"tiny"` }
...
func (s *PexelsSearchService) Search(ctx context.Context, itemName string, num int) ([]PhotoMatch, error) {
    ...
    if err != nil { return nil, fmt.Errorf("pexels request build: %w", err) }
    req.Header.Set("Authorization", s.apiKey)
    resp, err := s.client.Do(req)
    if err != nil { return nil, fmt.Errorf("pexels request: %w", err) }
    defer resp.Body.Close()
    if resp.StatusCode != http.StatusOK { return nil, fmt.Errorf("pexels status %d", resp.StatusCode) }
    var out pexelsSearchResponse
    if err := json.NewDecoder(resp.Body).Decode(&out); err != nil { return nil, fmt.Errorf("pexels decode: %w", err) }
