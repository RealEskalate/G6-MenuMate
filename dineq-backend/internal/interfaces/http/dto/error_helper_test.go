...
func TestNormalizeError_Domain(t *testing.T) {
	...
	if resp.Code != "email_already_in_use" {
		t.Fatalf("unexpected code %s", resp.Code)
	}
	if resp.Message != domain.ErrEmailAlreadyInUse.Error() {
...
func TestNormalizeError_DuplicateKey(t *testing.T) {
	...
	if resp.Field != "email" {
		t.Fatalf("expected field email got %s", resp.Field)
	}
	if resp.Code != "email_already_in_use" {
		t.Fatalf("expected code email_already_in_use got %s", resp.Code)
	}
}
