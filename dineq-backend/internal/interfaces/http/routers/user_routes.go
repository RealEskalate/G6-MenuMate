...
func NewUserRoutes(env *bootstrap.Env, group *gin.RouterGroup, db mongo.Database) {
	...
	auditLogRepo := repositories.NewAuditLogRepository(db, env.AuditLogCollection)
	auditLogUsecase := usecase.NewAuditLogUsecase(auditLogRepo, ctxTimeout)
	...
	group.GET("/me", middleware.AuthMiddleware(*env), userController.Me)
	group.GET("/users/:id", userController.GetPublicUser)
	group.GET("/users/avatar-options", userController.GetAvatarOptions)
