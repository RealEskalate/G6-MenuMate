...
func makeJWT(t *testing.T, env *bootstrap.Env, sub string) string {
    ...
    if err != nil { t.Fatalf("failed to sign token: %v", err) }
    return s
}
...
func TestRestaurantLifecycle_RedirectAndDeletion(t *testing.T) {
    ...
    if created.Slug == "" || created.ID == "" { t.Fatalf("expected slug & id in create response: %+v", created) }
