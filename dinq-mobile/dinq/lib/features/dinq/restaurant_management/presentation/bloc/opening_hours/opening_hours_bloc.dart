...
class OpeningHoursBloc extends Bloc<OpeningHoursEvent, OpeningHoursState> {
  ...
  void _onLoadOpeningHours(LoadOpeningHours event, Emitter<OpeningHoursState> emit) async {
  ...
  void _onUpdateOpeningTime(UpdateOpeningTime event, Emitter<OpeningHoursState> emit) {
    final currentState = state;
    if (currentState is OpeningHoursLoaded) {
      final updatedOpeningTimes = List<TimeOfDay?>.from(currentState.openingTimes);
  ...
  void _onUpdateClosingTime(UpdateClosingTime event, Emitter<OpeningHoursState> emit) {
    final currentState = state;
    if (currentState is OpeningHoursLoaded) {
      final updatedClosingTimes = List<TimeOfDay?>.from(currentState.closingTimes);
  ...
  void _onAddSpecialDay(AddSpecialDay event, Emitter<OpeningHoursState> emit) {
    final currentState = state;
    if (currentState is OpeningHoursLoaded) {
      final updatedSpecialDays = List<SpecialDay>.from(currentState.specialDays);
      updatedSpecialDays.add(SpecialDay(
        date: event.date,
        title: event.title,
        status: event.status,
      ));
  ...
  void _onRemoveSpecialDay(RemoveSpecialDay event, Emitter<OpeningHoursState> emit) {
    final currentState = state;
    if (currentState is OpeningHoursLoaded) {
      final updatedSpecialDays = List<SpecialDay>.from(currentState.specialDays);
  ...
  void _onSaveOpeningHours(SaveOpeningHours event, Emitter<OpeningHoursState> emit) async {
    final currentState = state;
    if (currentState is OpeningHoursLoaded) {
